...
int main(int argc, char* argv[]) try		// // //
{
	...
	auto printSongs = []() {
		...
		for (int i = 0; i < textFilesToCompile.size(); i++)
			fprintf(stderr,
				"song %d: %lu samples\n",
				firstLocalSong + i, global_musics[firstLocalSong + i].mySamples.size()
			);
		fprintf(stderr, "\n");
	};
...
void fixMusicPointers()
{
	...
	fprintf(stderr, "end of program = %d\n", global_programPos + global_programSize);
	if (global_programPos + global_programSize != songDataARAMPos)
	{
